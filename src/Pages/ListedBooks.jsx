...
const ListedBooks = () => {
  ...
  const handleReadSort = (e) => {
    const localStorageBooks =
      JSON.parse(localStorage.getItem("readedBook")) || [];
    const localStorageWishBooks =
      JSON.parse(localStorage.getItem("wishbook")) || [];
    // console.log("hi");
    if (localStorageBooks.length === 0 && localStorageWishBooks.length === 0) {
      ...
      localStorageBooks.length === 0 &&
      localStorageWishBooks.length > 0
    ) {
    ...
    if (e === "Rating") {
      const sortLoclStorageBooks = localStorageBooks.sort(
        (a, b) => b.rating - a.rating
      );
      const sortLoclStorageWishBooks = localStorageWishBooks.sort(
        (a, b) => b.rating - a.rating
      );
      localStorage.setItem("readedBook", JSON.stringify(sortLoclStorageBooks));
      localStorage.setItem(
        "wishbook",
        JSON.stringify(sortLoclStorageWishBooks)
      );
    } else if (e === "Number of Pages") {
      const sortLoclStorageBooks = localStorageBooks.sort(
        (a, b) => b.totalPage - a.totalPage
      );
      const sortLoclStorageWishBooks = localStorageWishBooks.sort(
        (a, b) => b.totalPage - a.totalPage
      );
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="text-center lg:mt-8 mt-4">
        <div className="dropdown">
          ...
          <ul
            onClick={() => {
              handleReadSort(event.target.innerText);
            }}
            tabIndex={0}
